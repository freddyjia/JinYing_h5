...
import { NetMsgDefine } from "../../Protos/MessageDefine";
...
export default class TCPSession {
            ...
            reader.onload = (e) => {
                ...
                for (let i = 0; i < packageArray.length; i++) {
                    ...
                    if (packageGet.seq != 0) {
                       ...
                       // cc.log("listenData", listenData)
                        if (listenData != null) {
                            if (listenData.gameType != GameTypeMgr.Config.None.gameType && outside.gameType != listenData.gameType) {
                                return;
                            }
    ...
    public HandShake(successCallback, failCallback) {
        ...
        this.Send(NetMsgDefine.HANDSHAKE, senddata, (data) => {
            ...
            if (res.statusCode == 0) {
                ...
                outside.Disconnect();
                MessageCenter.SendMessage(MessageNames.ReGetToken);
            }
    ...
    public SetUidToker(uid, token) {
        this.uid = uid;
        this.token = token;
    }

    public Send(msgID, data, successCallback, failCallback, hideNetworkingLoading) {
        this.SendWithGameType(GameTypeMgr.Config.None.gameType, msgID, data, successCallback, failCallback, hideNetworkingLoading)
    }

    public SendWithGameType(gameType, msgID, data, successCallback: (data: Uint8Array) => void, failCallback: (errMsg: string) => void, hideNetworkingLoading = false) {
        if (this.gameType != gameType && gameType != GameTypeMgr.Config.None.gameType) {
            return;
        }
        ...
        if (this.webSocket == null) {
            ...
            this.Connect(this.ip, this.port, () => {
                ...
                //因为断网挂起的消息，只会挂起一个
                this.hangOnSendFunc = () => {
                    this.SendWithGameType(gameType, msgID, data, successCallback, failCallback, hideNetworkingLoading);
                };

                MessageCenter.SendMessage(MessageNames.SendReConnectMsg);
                MessageCenter.SendMessage(MessageNames.TryToCloseDialogWithContent, ConstStr.ShouldLogout);
            }, () => {
                if (Date.now() - this.lastShowToastUI > 1000 * 10) {
                    this.lastShowToastUI = Date.now();
                    Toast.Show("连接失败,请检查网络环境");
                }
                this.Disconnect();
                MessageCenter.SendMessage(MessageNames.ReconnectSocketFail);
        ...
        if (this.isHandShake == false && msgID != NetMsgDefine.HANDSHAKE) {
    ...
    public SendOneWayWithGameType(gameType, msgID, data) {
        if (this.webSocket == null)
            return;
    ...
    public ListenMsgWithGameType(gameType, msgID, func) {
        let listenData = new ListenData();
        listenData.func = func;
        listenData.gameType = gameType;
    ...
    public SetCurrentGameType(gameType) {
